...
public class FrutasCad extends javax.swing.JFrame {
    ...
    public FrutasCad(Frutas janelaFrutas, int codigo) {
        ...
        if(codigo != -1) {
            ...
            if(db.next()) {
                String cor = db.getString("tipo");
                String nome = db.getString("nome");
                String sabor = db.getString("origem");
                String tamanho = db.getString("comprimento");
                txtCor.setText(cor);
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        String tipo = txtCor.getText();
        String origem = txtSabor.getText();
        String comprimento = txtTamanho.getText();
        DB db = new DB("bancodados.db");
        String query = "";
        if(codigo == -1) {
            query = "INSERT INTO frutas (nome, origem, tipo, comprimento) ";
            query = query + "VALUES (";
            query = query + "'" + nome + "',";
            query = query + "'" + origem + "',";
            query = query + "'" + tipo + "',";
            query = query + "'" + comprimento + "'";
            query = query + ");";            
        } else {
            query = "UPDATE frutas SET ";
            query = query + "nome='" + nome + "', ";
            query = query + "origem='" + origem + "', ";
            query = query + "tipo='" + tipo + "', ";
            query = query + "comprimento='" + comprimento + "'";
